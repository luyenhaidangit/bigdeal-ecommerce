...
@section('content')
                    ...
                    <div class="breadcrumb-contain">
                        <div>
                            ...
                            <ul>
                                ...
                                <li><a
                                        href="{{ route('guest.product.show', ['slug' => $product->slug]) }}">{{ $product->name }}</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="product-right">
                            <div class="pro-group">
                                ...
                                @if ($price)
                                    @if ($priceDiscount)
                                        <ul class="pro-price">
                                            <li>{{ number_format($priceDiscount, 0, ',', '.') }}đ</li>
                                            <li><span>{{ number_format($price, 0, ',', '.') }}đ</span></li>
                                            <li>Giảm {{ $discountPercentage }}%</li>
                                        </ul>
                                    @else
                                        <ul class="pro-price">
                                            <li>{{ number_format($price, 0, ',', '.') }}đ</li>
                                        </ul>
                                    @endif
                                @else
                                    <ul class="pro-price">
                                ...
                                <div class="revieu-box">
                                    ...
                                    <a href="review.html"><span>({{ count($product->productReviews) }} đánh giá)</span></a>
                                </div>
                            ...
                            @if ($timeRemaining)
                                <div class="pro-group">
                                    <h6 class="product-title">Nhanh tay ! Ưu đãi kết thúc trong <span
                                            class="text-danger">{{ $timeRemaining }}</span> nữa</h6>
                                </div>
                            @endif
                            ...
                            @if (count($product->productOptions) > 0)
                                <div id="selectSize"
                                    ...
                                    <h6 class="product-title size-text">Tuỳ chọn sản phẩm</h6>
                                    <div class="size-box">
                                        <ul>
                                            @foreach ($product->productOptions as $item)
                                                <li class="px-4 py-2 mb-2 ms-2 border-danger {{ $productOption->id == $item->id ? 'bg-danger' : '' }} {{ $productOption->id == $item->id ? 'text-white' : 'text-danger' }}"
                                                    style="width: unset !important;height: unset !important"
                                                    data-option-id="{{ $item->id }}">
                                                    @if ($item->color)
                                                        <span class="d-block" style="width: unset !important">Màu:
                                                            {{ $item->color }}</span>
                                                    @endif
                                                    <!-- Các phần tử option khác -->
                                                    @if ($item->size)
                                                        <span class="d-block" style="width: unset !important">Kích thước:
                                                            {{ $item->size }}</span>
                                                    @endif
                                                    @if ($item->ram)
                                                        <span class="d-block" style="width: unset !important">Ram:
                                                            {{ $item->ram }}</span>
                                                    @endif
                                                    @if ($item->rom)
                                                        <span class="d-block" style="width: unset !important">Rom:
                                                            {{ $item->rom }}</span>
                                                    @endif
                                                    @if ($item->ram_rom)
                                                        <span class="d-block" style="width: unset !important">Ram-Rom:
                                                            {{ $item->ram_rom }}</span>
                                                    @endif
                                                    @if ($item->cpu)
                                                        <span class="d-block" style="width: unset !important">CPU:
                                                            {{ $item->cpu }}</span>
                                                    @endif
                                                    @if ($item->sweep_frequency)
                                                        <span class="d-block" style="width: unset !important">Tần số:
                                                            {{ $item->sweep_frequency }}</span>
                                                    @endif
                                                    @if ($item->hard_drive)
                                                        <span class="d-block" style="width: unset !important">Ổ cứng:
                                                            {{ $item->hard_drive }}</span>
                                                    @endif
                                                    @if ($item->resolution)
                                                        <span class="d-block" style="width: unset !important">Độ phân
                                                            giải:
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="modal-bg addtocart">
                                    ...
                                    <div class="media">
                                        <a href="javascript:void(0)">
                                            <img src="../assets/images/layout-4/product/3.jpg" alt="cart-image"
                                                class="img-fluids">
                                        </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Lắng nghe sự kiện click trên các option
            const optionItems = document.querySelectorAll('#selectSize li[data-option-id]');
            optionItems.forEach(function(optionItem) {
                optionItem.addEventListener('click', function(event) {
                    event.preventDefault(); // Ngăn chặn hành vi chuyển đến đường dẫn href mặc định

                    const optionId = optionItem.getAttribute('data-option-id');
                    const currentUrl = URI(window.location.href);

                    // Thêm param 'code' với giá trị là id của option vào URL và chuyển hướng trang
                    const newUrl = currentUrl.setQuery('code', optionId);
